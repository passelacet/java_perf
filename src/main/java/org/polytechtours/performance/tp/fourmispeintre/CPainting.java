...
public class CPainting extends Canvas implements MouseListener {
    ...
    // differents tableaux cache des couleurs utilisées
    //private ArrayList<Color> listColor = new ArrayList<Color>();
    private HashMap<Integer, Color> listColor = new HashMap<>();
    private Color[][][] tabColor = new Color[256][256][256];

    /**
     * Initialise la matrice de toutes les objets Color possibles
     */
    private void initColor() {
        for (int i = 1; i < 256; i++) {
            for (int j = 1; j < 256; j++) {
                for (int k = 1; k < 256; k++) {
                    tabColor[i - 1][j - 1][k - 1] = new Color(i, j, k);
                }
            }
        }
    }
    ...
    /**
     * Vérifie si la couleur demandée existe déjà alors il la retourne sinon il la créé et la retourne
     * Utilise une ArrayList pour stocker les objets Color
     * @param r
     * @param g
     * @param b
     * @return l'objet Color associé à r, g et b
     */
    ...
    /**
     * Vérifie si la couleur demandée existe déjà alors il la retourne sinon il la créé et la retourne
     * Utilise une HashMap qui est plus adapté pour la mise en cache
     * @param r
     * @param g
     * @param b
     * @return l'objet Color associé à r, g et b
    ...
    /**
     * Vérifie si la couleur demandée existe déjà alors il la retourne sinon il la créé et la retourne
     * Utilise d'un tableau avec pour dimensions les couleurs
     * @param r
     * @param g
     * @param b
     * @return l'objet Color associé à r, g et b
     */
    private Color verifyColor(int r, int g, int b) {
        if(tabColor[r-1][g-1][b-1] == null){
          tabColor[r-1][g-1][b-1] = new Color(r,g,b);
        }

        return tabColor[r - 1][g - 1][b - 1];
    }
    ...
    public void init() {
        ...
        //initColor(); //si initialisation de tabColor
    }
